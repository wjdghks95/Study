...
public class ConfigurableDispatcherServlet extends DispatcherServlet {
    ...
    // 설정 방법을 확장해서 클래스와 XML 파일 두 가지 방법을 모두 지원하게 만들었따.
    private Class<?>[] classes;
    private String[] locations;

    private ModelAndView modelAndView; // 컨트롤러가 DispatcherServlet에 돌려주는 모델과 뷰 정보를 저장해서 테스트에서 확인할 수 있게 해준다.
    ...
    // DispatcherServlet의 서블릿 컨텍스트를 생성하는 메소드를 오버라이드 해서 테스트용 메타정보를 이용해서 서블릿 컨텍스트를 생성하게 했다.
    @Override
    ...
    // 뷰를 실행하는 과정을 가로채서 컨트롤러가 돌려준 ModelAndView 정보로 따로 저장해둔다.
